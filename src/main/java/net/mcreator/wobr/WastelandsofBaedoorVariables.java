...
public class WastelandsofBaedoorVariables {
	public static double Growth_Stadium = 0;
	public static class MapVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				WastelandsofBaedoor.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				WastelandsofBaedoor.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		...
		public boolean Reapering = false;
		...
		@Override
		public void readFromNBT(NBTTagCompound nbt) {
			Reapering = nbt.getBoolean("Reapering");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("Reapering", Reapering);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				WastelandsofBaedoor.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				WastelandsofBaedoor.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					WastelandsofBaedoor.PACKET_HANDLER.sendToAll(message);
				else
					WastelandsofBaedoor.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
